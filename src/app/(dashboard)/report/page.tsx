...
export default async function ReportPage({ searchParams }: Props) {
  ...
  const [{ data: profile }, { data: expenses }, { data: prevExpenses }] = await Promise.all([
  ...
  const goalAchieved = savingGoal > 0 && actualSaved >= savingGoal
  const savingPct = savingGoal > 0 ? Math.round((actualSaved / savingGoal) * 100) : 0
    ...
    <div className="min-h-screen pb-20" style={{ background: 'var(--color-bg)' }}>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <a href={`/report?month=${dayjs(currentMonth).add(1,'month').format('YYYY-MM')}`}
            className={`w-8 h-8 flex items-center justify-center rounded-full bg-white border border-gray-200 text-sm ${currentMonth >= dayjs().format('YYYY-MM') ? 'text-gray-200 pointer-events-none' : 'text-gray-600'}`}>›</a>
        </div>
      </div>
      ...
      <div className="rounded-2xl p-5 mb-4 text-white" style={{ background: goalAchieved ? '#10B981' : 'var(--color-primary)' }}>
        ...
        <p className="text-2xl font-bold mb-3">
          {goalAchieved ? '🎉 저축 목표 달성!' : savingGoal > 0 ? `목표까지 ₩${(savingGoal - actualSaved).toLocaleString()}` : '이번 달 소비 내역'}
        </p>
        <div className="flex gap-6 text-sm">
          ...
          {income > 0 && (
            <div>
              ...
              <p className="font-bold">₩{actualSaved.toLocaleString()}</p>
            </div>
          )}
          {savingGoal > 0 && (
            <div>
      ...
      {catTotals.length > 0 && (
        <div className="bg-white rounded-2xl p-4 border border-gray-100 mb-4">
          <p className="text-xs text-gray-400 mb-3">지출 TOP {Math.min(catTotals.length, 3)}</p>
      ...
      {topDay && (
        <div className="bg-white rounded-2xl p-4 border border-gray-100 mb-4">
          <p className="text-xs text-gray-400 mb-2">이달 최다 지출일</p>
          <div className="flex justify-between items-center">
            <p className="font-semibold text-gray-800">{dayjs(topDay[0]).format('M월 D일 (dd)')}</p>
