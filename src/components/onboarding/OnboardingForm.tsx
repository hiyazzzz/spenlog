  ...
  async function handleFinish() {
    ...
    try {
      ...
      const { error: uErr } = await supabase.from('users').upsert(
        { id: userId, email: email, name: finalName, income: parseMan(income), saving_goal: parseMan(goal), theme, onboarding_completed: true },
        { onConflict: 'id' }
      )
      if (uErr) throw uErr
        ...
        <div style={{ maxWidth: 420, margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '10px' }}>
            <button onClick={async () => {
              await supabase.from('users').upsert({ id: userId, email: email, onboarding_completed: true }, { onConflict: 'id' })
